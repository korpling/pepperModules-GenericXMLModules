...
public class XML2SaltMapper extends PepperMapperImpl {
    ...
    private class XMLReader extends DefaultHandler2
    {
	    ...
	    private class ElementNodeEntry
	    {
	        ...
	        /**
	         * Returns if element node is a complex node (contains further element nodes)
	         * @return
	         */
		 ...
		 * name of the element-node given by <em>nodeName</em> and returns them.
		 * @param nodeName
		 * @param attributes
		 * @return
		 */
			...
			{//create annotation list
				...
				{//if element-node shall not be ignored
					...
					{ 
						...
						if (	(((GenericXMLImporterProperties)getProperties()).isIgnoreNamespaces())&&
	            					...
	            					Attributes attributes)throws SAXException
	    {
			...
			else if (	(!this.matches(((GenericXMLImporterProperties)getProperties()).getIgnoreList(), currentXPath))&&
						(!((GenericXMLImporterProperties)getProperties()).isTextOnly()))
			{//if element-node shall not be ignored
