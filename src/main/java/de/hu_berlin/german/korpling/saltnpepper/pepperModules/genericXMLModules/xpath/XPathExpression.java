...
/**
 * Splits an XPath expression into steps. The main function of this class is to compare two XPath expressions,
 * if one expression is mathes the other one. For instance the XPath expression //element1 matches the XPath expression
 * element2/element1. Only wildcards are resolved and only shortcut syntax for descendant axis is supported. 
 *
...
public class XPathExpression {
	...
	/**
	 * Checks if the given XPath expression matches the expression hold by this object.
	 * @param xPathExpression
	 * @return
	 */
	...
	/**
	 * Checks if the given XPath expression matches the expression hold by this object.
	 * @param xPathExpression1 the one containing wildcards
	 * @return
	 */
		...
		while (goOn)
		{
			...
			{// if current step in xPathExpression1 is wildcard
				...
				{//wildcard is last step of xPathExpression1
					// if last step in xPathExpression2 is attribute or text node expressions do not match 
					if (	(xPathExpression2.getSteps().get(xPathExpression2.getSteps().size()-1).startsWith("@"))||
							("text()".equals(xPathExpression2.getSteps().get(xPathExpression2.getSteps().size()-1))))
						equal= false;
					else equal= true;
					break;
				}//wildcard is last step of xPathExpression1
	...
	/**
	 * Adds a step to the current expression. the new step will be set to the end.
	 * @param step
	 */
